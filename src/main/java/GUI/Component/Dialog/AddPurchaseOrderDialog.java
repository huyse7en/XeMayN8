...
public class AddPurchaseOrderDialog extends JDialog {
    ...
    if (employeeId.isEmpty()) {
        ...
        currentEmployee = null; // Đảm bảo reset khi không có mã NV
        return;
    }
    ...
    try {
        ...
        employeeNameLabel.setText("    Tên NV: [Lỗi tìm kiếm]");
...
//        }
    ...
    private boolean validateInput() {
        ...
        if (currentEmployee == null) {
            JOptionPane.showMessageDialog(this, 
                "Nhân viên không hợp lệ hoặc không được chọn", 
                "Lỗi", 
    ...
    try {
        ...
        if (success) {
            ...
            for (PurchaseOrderDetailDTO detail : pendingOrderDetails) {
                detail.setMaPN(currentPurchaseId);
