...
import DAO.PurchaseStatisticsDAO;
...
public class PurchaseStatisticsBUS {
    private PurchaseStatisticsDAO purchaseStatisticsDAL;

    public PurchaseStatisticsBUS() {
        purchaseStatisticsDAL = new PurchaseStatisticsDAO();
    }
    ...
    public String countBook() {
        String count = "";
        try {
            count = purchaseStatisticsDAL.sumProductPurchase().toString();
        } catch (Exception e) {
            e.printStackTrace();
        }
        return count;
